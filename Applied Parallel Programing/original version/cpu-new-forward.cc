...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
  ...
  // Insert your CPU convolution kernel code here
	for(int b_id = 0; b_id < B; b_id++){                    // for each image in the batch 
		for(int m_id = 0; m_id < M; m_id++){                 // for each output feature maps
			for(int h_id = 0; h_id < H_out; h_id++){         // for each output element
				for(int w_id = 0; w_id < W_out; w_id++){
					y4d(b_id, m_id, h_id, w_id) = 0;
					for(int c_id = 0; c_id < C; c_id++){     // sum over all input feature maps
						for(int p = 0; p < K; p++){ // KxK filter
							for(int q = 0; q < K; q++)
								y4d(b_id, m_id, h_id, w_id) += x4d(b_id, c_id, h_id + p, w_id + q) * k4d(m_id, c_id, p, q);
						}
					}
				}
			}
